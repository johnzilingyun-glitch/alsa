 ...
 * Falls back to a UTC ISO date string if the timezone is not supported.
 */
...
export function generateHistoryItemKey(item: any, index: number): string {
  if (!item || typeof item !== 'object') return `history-unknown-no-time-${index}`;
  if (item.id) return item.id;
  const symbol = item.stockInfo?.symbol || 'unknown';
  const time = (item.stockInfo?.lastUpdated || 'no-time').toString().replace(/\s+/g, '_');
