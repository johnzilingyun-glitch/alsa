...
3. **均线系统分析 (NEW)**: 你必须参考 [API数据] 中的 MA5, MA20, MA60。分析当前价格与各均线的相对位置（站上/跌破），以及均线排布形态（多头排列/空头排列/缠绕）。
...
1. 评估核心财务指标（PE/PB/ROE/净利润增长率/营收/分红率）。必须参考 [API数据] 中提供的最新净利润增速及分红数据。
2. 与同行业公司估值对比
3. 分析盈利质量 and 可持续性
4. 给出合理估值区间，并**显式引用 [API数据] 中的估值方法论 (methodology)**，向用户解释计算依据（如格雷厄姆原则、戈登增长模型等）。
...
要求：
- 指标至少包括：北向资金净流入 (Northbound)、融资买入/偿还额 (Margin)、龙虎榜机构净买入 (LHB)、社媒舆情热度 (Social Trends)
- 必须优先使用 [API数据] 中提供的深度维度行情（北向资金、龙虎榜、两融、社媒数据）。若缺失则使用 Google Search 获取最新数据。
